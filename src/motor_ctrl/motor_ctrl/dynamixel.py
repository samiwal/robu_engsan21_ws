...
class Dynamixel:
    ...
    def set_goal_velocity(self, motor_id, goal_velocity):
        dxl_comm_result, dxl_error = self.packetHandler.write4ByteTxRx(self.portHandler, motor_id,
                                                                       self.ADDR_GOAL_VELOCITY, goal_velocity)
    
    def get_velocity_limit(self, motor_id):
        limit = self._read_value(motor_id, ReadAttribute.VELOCITY_LIMIT, 4)
        if limit > 2 ** 31:
            limit -= 2 ** 32
        return limit
